...
public static class DbContextExtensions
{
    ...
    /// <summary>
    /// Promotes the user with <paramref name="email"/> to the <c>Admin</c> role
    /// using the same <see cref="UserManager{TUser}"/> API the production
    /// <see cref="Seed"/> uses. Mirrors the role-flip the test helper used to
    /// do inline; centralising it here keeps the role-management code out of
    /// every test file.
    ///
    /// Call <b>after</b> registering the user via <c>POST /api/auth/register</c>
    /// (which auto-assigns the <c>Customer</c> role on the controller side),
    /// then <c>POST /api/auth/login</c> to obtain an Admin JWT.
    /// </summary>
    public static async Task SeedAdminAsync(this ApiFactory factory, string email)
    {
        using var scope = factory.Services.CreateScope();
        ...
        var user = await userManager.FindByEmailAsync(email)
            ?? throw new InvalidOperationException(
                $"SeedAdminAsync: no user with email {email} exists. Register the user first.");

        // Remove the Customer role added by AuthController.Register, then add
        // Admin. This matches the production flow in Data/Seed.cs (single role
        // per user) and ensures the JWT "role" claim is "Admin" (not "Customer").
    ...
    /// when a test class needs catalog data. Each product gets a default
    /// LEGACY variant with the original stock value — ADR 0003 (Task 27).
    /// </summary>
