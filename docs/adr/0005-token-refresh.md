...
Access tokens are short-lived JWTs (60-minute default). Refresh tokens are long-lived, single-use-per-rotation, server-side state, delivered as an `httpOnly`, `SameSite=Lax` cookie scoped to `/api/auth` with `Secure=true` only in Production — `Secure=false` in Development, Staging and Testing so the cookie is not dropped over plain HTTP (see `RefreshCookieOptions.ShouldUseSecure`). The frontend axios interceptor calls `POST /api/auth/refresh` on any 401 from a request that *was* authenticated; the response is a new access token (and a new refresh token via cookie rotation), and the original request is retried once.
...
**Status (2026-08-20):** **SHIPPED** in `8a640dd` follow-up — `RefreshToken` + `RefreshTokenService` (hash storage, rotation via `RevokedAt`/`ReplacedById`), `POST /api/auth/refresh` and `POST /api/auth/logout` (httpOnly `SameSite=Lax` cookie scoped to `/api/auth`), frontend `api.ts` 401→refresh→retry-once, `Jwt:RefreshExpiresInDays=30`, 6 integration tests. Cross-tab race is first-wins, second 401 as documented.
