...
**Status (2026-08-20):** **SHIPPED** in `8a640dd` (`fix(api): atomic stock deduction + config-driven shipping + Swagger annotations`). `OrdersController.Checkout` now uses `ExecuteSqlInterpolatedAsync` per cart item (`UPDATE "Products" SET "Stock" = "Stock" - @qty WHERE "Id" = @id AND "Stock" >= @qty`, gating on `rowsAffected`) with explicit restock loops for `INSUFFICIENT_STOCK` and `PAYMENT_FAILED`. `ShippingOptions` (`Shipping:Fee`) and Swagger annotations also shipped in the same commit. InMemory fallback preserves test correctness; Postgres concurrency path still lacks a true concurrent integration test (requires Postgres/Testcontainers) — tracked as a hardening follow-up.
